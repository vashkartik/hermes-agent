...
export function useComposerMetrics({ composerRef, composerSurfaceRef, editorRef, poppedOut }: UseComposerMetricsArgs): {
  ...
  // mobile/coarse-pointer embeds are forced docked and must keep publishing
  // docked metrics even when the persisted atom says popped out).
  ...
  const syncComposerMetrics = useCallback(() => {
    ...
    // poppedOutRef (set above) keeps the callback stable AND tracks the full
    // eligibility gate. Upstream's inline $composerPoppedOut.get() &&
    // !isSecondaryWindow() covers only secondary windows; the ref also forces
    // docked metrics for mobile/coarse-pointer embeds, so keep the ref.
