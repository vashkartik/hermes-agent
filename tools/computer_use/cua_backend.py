...
def _macos_cua_app_bundle(driver_cmd: str) -> Optional[str]:
    """Return the installed CuaDriver.app containing *driver_cmd*.

    A cua daemon spawned directly by an Electron-hosted Hermes process inherits
    that Electron app as macOS's TCC ``responsible`` identity.  Local ad-hoc
    Electron rebuilds are CDHash-bound, so their otherwise-visible Accessibility
    and Screen Recording grants stop matching after every rebuild.  Launching the
    same private daemon through LaunchServices gives it the stable, Developer-ID
    signed ``com.trycua.driver`` identity instead.

    Fail closed unless the resolved binary is actually inside the expected app
    bundle.  Falling back to a direct child would silently recreate the permission
    loop this path exists to prevent.
    """
    if sys.platform != "darwin":
        return None
    ...
    try:
        ...
        return None
    if info.get("CFBundleIdentifier") != "com.trycua.driver":
        return None
    return app_path
...
class _EmbeddedCuaDaemon:
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        self._launch_stderr_path: Optional[str] = None
    ...
    def start(self) -> None:
        ...
        if self.permission_mode == "unrestricted":
            daemon_args.append("--dangerously-bypass-approvals")
        ...
        command = daemon_args
        if sys.platform == "darwin":
            app_path = _macos_cua_app_bundle(self._command)
            if not app_path:
                raise RuntimeError(
                    "cua-driver private permission modes on macOS require the signed "
                    "CuaDriver.app bundle; reinstall it with `hermes computer-use install`"
                )
            self._launch_stderr_path = f"{self.socket_path}.stderr.log"
            command = [
                "/usr/bin/open",
                "-W",
                "-g",
            ...
            command.extend([app_path, "--args", *daemon_args[1:]])
        ...
        while time.monotonic() < deadline:
            if self._process.poll() is not None:
                detail = self._startup_diagnostic()
                self.stop()
                raise RuntimeError(
        ...
        detail = self._startup_diagnostic("daemon did not become ready")
        self.stop()
        raise RuntimeError(f"embedded cua-driver startup timed out: {detail}")

    def _startup_diagnostic(self, fallback: str = "no diagnostic output") -> str:
        detail = "; ".join(self._stderr_tail)
        if self._launch_stderr_path:
            try:
                with open(self._launch_stderr_path, encoding="utf-8", errors="replace") as handle:
                    app_detail = handle.read()[-4000:].strip()
                if app_detail:
                    detail = f"{detail}; {app_detail}" if detail else app_detail
            except OSError:
                pass
    ...
    def stop(self) -> None:
        ...
        if self._launch_stderr_path:
            try:
                os.remove(self._launch_stderr_path)
            except OSError:
                pass
            self._launch_stderr_path = None
