    ...
    for sid, session in owned:
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if heir is not None and not _transport_is_dead(heir):
                    ...
                    # stdio) so emits are dropped/replayed rather than hitting
                    # a dead socket; a standalone `hermes --tui` keeps real
                    # _stdio. UNLESS another window still shows the session:
                    # multi-window pop-outs all register as viewers, so on
                    # disconnect re-bind to the most recent surviving viewer.
                    viewers = current.get("viewers")
                    if viewers:
                        viewers.pop(transport, None)
                    remaining = [
                    ...
                    if remaining:
                        ...
                        promoted_owner = remaining[-1][1]
                        current["transport"] = promoted_owner
                    else:
        ...
        if promoted_owner is not None:
            _emit(
                ...
                sid,
                {"reason": end_reason, "client": _stream_client_label(promoted_owner)},
            )
...
_KANBAN_NOTIFY_KINDS = (
    ...
    "status", "archived", "unblocked", "block_loop_detected", "controller_envelope",
    ...
    *,
    ...
    if not can_start:
        ...
        if start_error is not None:
            _emit("error", sid, {"message": f"turn start failed: {start_error}"})
    return can_start
