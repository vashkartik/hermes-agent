...
def _(rid, params: dict) -> dict:
    ...
    def run_after_agent_ready() -> None:
        ...
        if err:
            ...
            with session["history_lock"]:
                session["running"] = False
                # NOTE: deliberately do NOT _clear_inflight_turn here — the
                # retained error snapshot is what lets resume replay this
                # failure to a client that was disconnected.
                session["last_active"] = time.time()
            _release_update_turn_if_idle(session)
        ...
        with session["history_lock"]:
            if session.get("_turn_cancel_requested") or not session.get("running"):
                ...
                release_idle = False
        # Released outside history_lock: the handle is detached under the lock
        # before the file operation (see _release_update_turn_if_idle).
        if release_idle:
            _release_update_turn_if_idle(session)
            return
