...
def test_prompt_submit_empty_truncation_allowed_with_confirm(monkeypatch):
    ...
    try:
        ...
        assert seen["history"] == []
        # Ace fork: _persist_session_history durably writes the completed turn at
        # turn end, so the truncation write is the FIRST of two here, not the
        # only one. Upstream has no turn-end persist.
        assert replaced[0] == ("session-key", [])
...
def test_prompt_submit_continues_after_auto_compression_rotates_session(monkeypatch):
    ...
    class _CompressingAgent:
        ...
        def run_conversation(
            ...
            task_id=None,
            # Upstream keeps adding turn kwargs (e.g. persist_user_message,
            # #70720); absorb them so this fork regression test keeps testing
            # the session-rotation behaviour it is actually about.
            **_kwargs,
        ):
...
def test_clarify_callback_has_no_wall_clock_timeout(monkeypatch):
    """Desktop clarify remains pending until answered or explicitly interrupted."""
    captured = {}
...
# NOTE (Ace fork): upstream's test_clarify_timeout_seconds_maps_non_positive_to_unlimited
# is intentionally NOT carried here. The fork removed _clarify_timeout_seconds so desktop
# clarify waits forever (see test_clarify_callback_has_no_wall_clock_timeout above and the
# 'clarify: desktop wait has no wall-clock timeout' entry in capella-patch-guard.sh).
