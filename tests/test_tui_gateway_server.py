...
def test_ws_disconnect_parks_running_turn_without_scheduling_orphan_reap(monkeypatch):
    callbacks = []
    interrupted = []
...
def test_close_transport_detaches_when_no_viewers_remain(monkeypatch):
    ...
    without killing the durable session while its turn can still finish."""
    reap_calls = []
    monkeypatch.setattr(server, "_schedule_ws_orphan_reap", lambda sid: reap_calls.append(sid))
