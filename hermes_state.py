...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def get_resume_conversations(
        ...
        - ``display_history`` — the full visible lineage (ancestors → tip),
          including in-place compaction archives, verbatim, with replayed-user
          dedup. Explicit ``/branch`` sessions are
    ...
    def get_ancestor_display_prefix(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        ancestor_ids = {
            int(row["id"])
            for row in rows
            if row["id"] is not None and (row["session_id"] != session_id or row["active"] == 0)
        }
        if not ancestor_ids:
            return []
